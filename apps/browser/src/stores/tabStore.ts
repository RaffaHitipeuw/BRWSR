...
// Refactored to use memoryManager for lifecycle decisions
  ...
  persist(
    (set, get) => ({
      ...
      removeTab: (id) => {
        ...
        // Save to closed tabs for Ctrl+Shift+T
        ...
        if (newTabs.length === 0) {
          // Signal app to close or create new tab
          return;
        }
      ...
      // P0-2: Memory-pressure-driven decisions
      enforceMemoryLimits: () => {
        const state = get();
        const memory_manager = useMemoryManager.getState();
...
// Closed tabs queue (for Ctrl+Shift+T)
let closedTabs: Tab[] = [];
