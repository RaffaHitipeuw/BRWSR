  ...
  persist(
    (set, get) => ({
      ...
      addPassword: (url, username, password, notes = "") => {
        ...
        set((state) => {
          let newPasswords = [newPassword, ...state.passwords];
          if (newPasswords.length > MAX_PASSWORDS) {
            newPasswords = newPasswords.slice(0, MAX_PASSWORDS);
          }
      ...
      importPasswords: (jsonData) => {
        try {
          ...
          set((state) => {
            let allPasswords = [...newPasswords, ...state.passwords];
            if (allPasswords.length > MAX_PASSWORDS) {
              allPasswords = allPasswords.slice(0, MAX_PASSWORDS);
            }
